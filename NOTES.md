...
- **Blockchain**: Uses `viem` + injected wallet provider (MetaMask) to implement wallet connect, a contract read (native ETH + ERC20 `balanceOf`), and contract writes (`deposit` + `transfer`) with tx status feedback and explorer links.

## What I would improve with more time

- **API auth UX**: Add a login form that supports cookie-based sessions and/or storing a token, so protected endpoints can be demonstrated without relying on env vars.
...
- **Blockchain UX**:
  ...
  - Display chain name (in addition to chain id).
